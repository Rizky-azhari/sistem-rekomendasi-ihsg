...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'stock_universe' AND policyname = 'Allow public read access on stock_universe') THEN
        CREATE POLICY "Allow public read access on stock_universe" ON public.stock_universe FOR SELECT TO anon, authenticated USING (true);
    END IF;
END $$;
