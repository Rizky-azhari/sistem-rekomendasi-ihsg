...
from app.schemas.recommendation import TradingPlan

def generate_trading_plan(df_indicators: pd.DataFrame, signal: str) -> TradingPlan:
    """
    ...
    # Calculate Entry Price & Entry Range
    if signal in ["STRONG BUY", "BUY"]:
        # Ideal entry near current price or pullback to SMA20
        ...
        # Take Profit 1 (R:R 1:1.5) and Take Profit 2 (R:R 1:3.0)
        ...
        notes = f"Buy setup active. Maintain strict risk management of max 2% total portfolio per trade."
        ...
        stop_loss = round(current_price + risk_distance, 0) # For short or exit trailing stop
